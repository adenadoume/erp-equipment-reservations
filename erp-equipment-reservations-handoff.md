...
### 2026-07-29 (continued further) — Persistent audit log; explained per-reservation SoftOne lines

User reported ΠΑΛ-ΑΝ0026 not reflecting new architect additions — re-checked with the full per-code quantity comparison (not just line count): **zero mismatches, 73 codes, 1,875 units on both sides**, confirmed correct at time of checking. User confirmed it was a timing/refresh issue on their end, not a real ongoing bug. While investigating, found a real gap worth closing anyway: I'd redeployed the backend that morning for the email feature, which restarts the container and wipes docker's log buffer — so there was no way to retroactively check whether a *specific* earlier sync call had succeeded or failed. Nothing before a redeploy was ever inspectable.

**Fixed**: added a persistent, file-based log that survives redeploys.
- `EQUIPMENT_LOG_PATH = /app/logs/equipment-events.log` inside the container, backed by a **docker volume mount** (`-v /home/ubuntu/softone-live/logs:/app/logs` — this is now a required flag on every `docker run` for this container, not optional; write it into muscle memory or check `docker inspect softone-live` for mounts before assuming a plain restart preserves logs). `_log_equipment_event(event, detail)` helper in `main.py`, called from `sync-stock`, `sync-order` (success/skip/fail), and the new endpoint below.
- **New `POST /api/equipment/log-event`** — user separately asked for "logs for every entry made by architects to be persistent... not huge project." Deliberately the simplest thing that works: one flat append-only log file, no new Supabase table, no new UI. Takes `{action, architect_name, project_code, product_code, quantity}`, writes one line. This is what actually fills the gap Supabase can't: `reservations` has `created_at`/`updated_at` for create/edit, but a **deleted** row leaves zero trace in Supabase — this log is the only record of "who deleted what" going forward.
- Frontend: `src/lib/logEvent.ts` (`logReservationEvent()`, fire-and-forget) called from `ReserveModal.tsx` (create), `Reservations.tsx` `saveEdit` (edit) and `deleteRow` (delete — signature changed from `(id)` to `(row)` since the full row's details are needed for the log line, not just its id).
- Verified persistence for real: wrote a test entry, confirmed it's readable directly from the **host** filesystem (`/home/ubuntu/softone-live/logs/equipment-events.log`), proving it survives the next container recreation (unlike a plain in-container file write, which docker's `stop && rm && run` cycle silently destroys every deploy).

**Also explained, per user's question — why does every reservation get its own SoftOne line, even when several reservations share the same product code?** This is intentional, not a bug. Rationale:
1. **1:1 traceability**: every Supabase `reservations` row maps to exactly one `softone_linenum`. Any line in SoftOne can be traced back to a specific reservation (who, which project, when) without ambiguity.
2. **Simple, safe update logic**: deleting or editing one reservation only ever touches its own line. Aggregating by code would mean every edit/delete needs to recompute a shared sum across potentially many reservations for that code — more moving parts, more chances for the "delete only" full-replace design to get a total wrong.
3. **No loss of total accuracy either way** — SoftOne's own `SUMAMNT`/stock accounting sums across all lines regardless of whether they share an item code, so order value and stock commitment math is identical whether it's 1 line or 5 lines for the same code.
4. Tradeoff: more lines in the document than a human might expect at a glance if many reservations share a code.

**Recommendation given to user**: keep per-reservation lines as-is. If a "total reserved per code" view is wanted for reviewing the document, that's better served as a separate reporting view (e.g. sort/group by item code in SoftOne itself, or a query against Supabase) than by collapsing the actual document structure — collapsing would trade away the traceability and simplicity benefits above for a cosmetic win. Not changed pending user's response to this.

### 2026-08-05 — Added 9 new I247 GEOCELL/geotextile products to the catalog

User dropped two supplier purchasing/costing spreadsheets into the repo root (`I247 GEOCELL - I110.15 ENTYPO PARALAVIS.xlsx`, 966-row template but only 7 real data rows, and `I247.1 GEOCELL ENTYPO PARALAVIS.xlsx`, 7 rows) and asked to add the products to the catalog so architects can reserve them, with photos pulled from the spreadsheets themselves.

- Parsed both xlsx files with `openpyxl` — the "AROMA" `Code`/`Description` columns are the actual SoftOne item code/name (this spreadsheet's real purpose is landed-cost/pricing calculation for an import shipment, not a catalog export). Combined unique code set across both files: **9 codes** (`ZPVC001.1`, `ZPVC004.1`, `ZPVC004.2`, `ZPVC003`, `ZPVC005`, `ZPVC006`, `ZPVC007`, `ZPVC008`, `ZPVC002`) — geocell (with/without edge, two sizes), plastic connectors, cement blanket, woven/non-woven geotextile, drainage board, PVC membrane.
- **Confirmed all 9 codes already exist live in SoftOne** (per user's claim) by querying `ITEM` directly (reused `softone-live` backend's local credentials/`softone.py` from `/Users/nucintosh/PYTHON/softone-live/backend/`, not this app's own code) — pulled authoritative `ITEM.NAME` (description), `ITEM.PRICER` (retail price, missing for 2 of the 9 — fell back to the spreadsheet's own "PROPOSAL PRICE" column for those, which matched `PRICER` exactly for every other code, good cross-check), `ITEM.V21` (ΔΙΑΘΕΣΙΜΑ/available stock), and `MTRL` (parsed from `ZOOMINFO`) for each. Set both `q` and `stock_softone` to the live `V21` value since these are brand-new rows with zero reservations in this app yet (available = total, same reasoning as every other product at import time).
- **Images**: extracted embedded pictures directly from each xlsx's `xl/media/` + `xl/drawings/drawing1.xml` (anchor row → cell → image relationship, same technique as reading any OOXML drawing), matched by row anchor to product code, saved into `catalog-images/<KODIKOS>.<ext>` (e.g. `ZPVC001.1.png`) — same jsDelivr-CDN convention as the original 412 images. All 3 geocell variants share one photo (same product family, edge vs. no-edge isn't visually distinguishable in the supplied picture). Committed + pushed to `main`, then hit `purge.jsdelivr.net` for each new file to force an immediate CDN cache refresh instead of waiting on jsDelivr's normal cache window.
- **Wrote products directly to Supabase** (`POST .../rest/v1/products?on_conflict=kodikos` with `Prefer: resolution=merge-duplicates`) using the **service-role key** found in the `softone-live` backend's local `.env` (this app's own `.env.local` only has the anon key, and `products` has no authenticated-insert RLS policy — service-role was the only way to do this without asking the user to run SQL by hand). Category/subcategory set to `ΟΙΚΟΔΟΜΙΚΑ ΥΛΙΚΑ` (closest existing fit among the catalog's 21 categories; nothing earthworks/geotextile-specific existed), `promitheftis`/`container` set to `I247` (the supplier code SoftOne itself reports for all 9, and the shipment reference embedded in every item's own SoftOne name).
- **Did not commit the two source xlsx files** — same reasoning as the gitignored SoftOne BlackBook PDF: they contain supplier cost prices (USD), landed-cost math, and markup multipliers, which shouldn't sit in a public GitHub repo. Added `I247*.xlsx` to `.gitignore`.
- **This app's own code was not touched** — no new backend endpoint was built for this (the "single add product" flow flagged as queued-but-not-started back on 2026-07-27 is still not built); this was a one-off manual data load using credentials that happen to exist locally from the sibling `softone-live` project. If this needs to become a repeatable self-serve flow for the team, that's still an open task.

### 2026-08-05 (continued) — Project add-flow: OIK30/OIK115 added, confirm dialog everywhere

User asked to add `OIK30`/`OIK115` as project codes (referenced as the "Client" for several of the new I247 products above) and to be able to add a brand-new OIK project directly from the Reservations page too, with a confirmation step so typos don't silently create junk project codes.

- Inserted `OIK30` and `OIK115` into `projects` directly via the same service-role key used above.
- `Reservations.tsx`'s "Project" filter previously derived its dropdown options from **codes already present in loaded reservations** (`Array.from(new Set(rows.map(r => r.project_code)))`) — meaning a project with zero reservations (like these two, freshly added) would never show up there. Now fetches the actual `projects` table (`loadProjects()`, parallel to `load()`) and uses that as the source of truth for the dropdown instead. Since `reservations.project_code` has an FK to `projects.code`, every reservation's project is guaranteed to already be in this list — no need to merge both sources.
- Same `Select` is now `mode="tags"` (same pattern already used in `ReserveModal`'s project picker) — typing a code that doesn't exist yet triggers `Modal.confirm` ("Add new project? '<code>' doesn't exist yet...") before it's actually inserted into `projects` and set as the active filter. Cancelling leaves the filter unchanged (the `Select`'s value stays controlled by React state, so no stale tag lingers).
- **Added the same `Modal.confirm` step to `ReserveModal.tsx`'s existing "add project" flow** (reserving equipment against a brand-new OIK code), which previously inserted immediately with zero confirmation — same click-happy-typo risk (typing an OIK code either wrong or with a duplicate that looks slightly different, e.g. `OIK30` vs `OIK3O`). On cancel or insert error, `form.setFieldValue('project_code', [])` resets the picker rather than leaving a phantom unsaved selection.
- Verified `npm run build` (tsc + vite) succeeds with these changes; no lint errors introduced.
