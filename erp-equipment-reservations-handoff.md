...
> **CURRENT STATUS (2026-07-27): order-sync is DISABLED.** It was built, deployed, and verified working end-to-end (122 real reservation lines confirmed live in ΠΑΛ-ΑΝ0026 via readback) — then immediately paused by the user: some historical reservations were already manually synced into SoftOne through other means, and this sync is a **full-replace**, so running it risks overwriting/duplicating that manual work. `triggerOrderSync()` in `src/lib/orderSync.ts` is a no-op until the user gives an explicit go-ahead — see section 4 for full detail. **Do not re-enable it without asking first**, even if it looks like an obviously-safe change.
